...
(function (window, document, $, angular) {
    ...
    registrationApp.controller("musicTypeCtrl", ["$scope", "$rootScope", "registrationSvcs", "registrationModel", "lookupStoreSvcs", "$state", "notifySvcs", "localStorage", "http", "$timeout", "modalSvcs", "typeData", function ($scope, $rootScope, registrationSvcs, registrationModel, lookupStoreSvcs, $state, notifySvcs, localStorage, http, $timeout, modalSvcs, typeData) {
        $rootScope.pageHeader = "MusicType";
        $rootScope.pageDescription = "Registration";

        var vm = this;
        vm.formModel = registrationModel.getInstance('musicType');
        vm.musicRoomList = typeData.MusicTypeList;
        ...
        vm.updateModal = function(instance){
            var reqMap = {
                eoMusicTypePK : instance.primaryKey +''
            };
            registrationSvcs.getMusicTypeByPk(reqMap, function(res){
                if(res.data != null){
        ...
        vm.deleteModal = function(d){
            ...
            bootbox.prompt({
                title: "Do You want to delete <span style='color: #283593; font-weight: 700'>"+ d.musicType + "</span> ?",
                locale: 'custom',
                callback: function (result) {
                    if(result != null){
                        var reqMap = {
                            eoMusicTypePK : d.primaryKey+'',
                            reason : result
                        };
                        registrationSvcs.deleteMusicTypeByPk( reqMap, function (res){
                            if(res.data == "Success"){
                                notifySvcs.success({
                            ...
                            if(res.data == "failure"){
                                notifySvcs.info({
                                    content : " Music Type is already in use"
                                });
                            }
                            $state.reload();
        ...
        vm.formConfig = {
            ...
            submit: function (e) {
                ...
                if (validationResponse.invalidInputs.length == 0) {
                    ...
                    registrationSvcs.createMusicType(blankFormInstance, function (res) {
                        if (res.data == 'Success') {
                            $state.reload();
                            notifySvcs.success({
                                title: "Music Type",
